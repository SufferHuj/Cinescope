...
class TestDBMovies:
    ...
    def test_db_movie_requests(self, db_helper, movie_test_data, created_test_user):
        ...
        try:
            ...
            retrieved_movie = db_helper.get_movie_by_id(created_movie.id)
    ...
    def test_movie_crud_operations(self, db_helper, movie_test_data):
        ...
        try:
            ...
            movie_by_name = db_helper.get_movie_by_name(created_movie.name)
            assert movie_by_name is not None
            assert movie_by_name.id == created_movie.id
            
            # Проверяем существование фильма
            assert db_helper.movie_exists_by_name(created_movie.name)
    ...
    def test_movie_filtering_methods(self, db_helper, movie_test_data):
        ...
        movie1 = db_helper.create_test_movie(movie_data_1)
        movie2 = db_helper.create_test_movie(movie_data_2)
        movie3 = db_helper.create_test_movie(movie_data_3)

        try:
            # Тестируем фильтрацию по жанру
            genre_1_movies = db_helper.get_movies_by_genre(1)
            ...
            price_range_movies = db_helper.get_movies_by_price_range(300, 600)
    ...
    def test_movie_existence_check(self, db_helper, movie_test_data):
        ...
        assert not db_helper.movie_exists_by_name(created_movie.name)
