...
class TestDBReviews:
    ...
    def test_review_crud_operations(self, db_helper, created_test_user, movie_test_data, review_test_data):
        ...
        # Создание отзыва
        created_review = db_helper.create_test_review(review_data)
        ...
        try:
            ...
            retrieved_review = db_helper.get_review_by_ids(
            ...
            assert db_helper.review_exists_by_ids(
                created_review.movie_id, 
                created_review.user_id
            )
            
            # Тестируем поиск несуществующего отзыва
            non_existent_review = db_helper.get_review_by_ids(99999, "fake-user-id")
            assert non_existent_review is None
            
            # Проверяем несуществование отзыва
            assert not db_helper.review_exists_by_ids(99999, "fake-user-id")
    ...
    def test_review_filtering_by_movie(self, db_helper, movie_test_data, review_test_data):
        ...
        for i in range(2):
            ...
            test_user = db_helper.create_test_user(user_data)
            ...
            created_reviews.append(db_helper.create_test_review(review_data))
        
        try:
            # Получаем все отзывы для фильма
            movie_reviews = db_helper.get_reviews_by_movie_id(created_movie.id)
    ...
    def test_review_update_operations(self, db_helper, created_test_user, movie_test_data, review_test_data):
        ...
        try:
            ...
            new_rating = 5
            update_success = db_helper.update_review_rating(
                created_review.movie_id, 
                created_review.user_id, 
                new_rating
            )
            ...
            updated_review = db_helper.get_review_by_ids(
                created_review.movie_id, 
                created_review.user_id
            )
            ...
            update_success = db_helper.update_review_text(
                created_review.movie_id, 
                created_review.user_id, 
                new_text
            )
            ...
            hide_success = db_helper.hide_review(
                created_review.movie_id, 
                created_review.user_id
            )
            ...
            # Получаем обновленный отзыв для проверки
            hidden_review = db_helper.get_review_by_ids(
                created_review.movie_id, 
                created_review.user_id
            )
            ...
            show_success = db_helper.show_review(
                created_review.movie_id, 
                created_review.user_id
            )
            ...
            # Получаем обновленный отзыв для проверки
            shown_review = db_helper.get_review_by_ids(
                created_review.movie_id, 
                created_review.user_id
            )
    ...
    def test_review_deletion(self, db_helper, created_test_user, movie_test_data, review_test_data):
        ...
        try:
            ...
            db_helper.delete_review(created_review)
            
            # Проверяем, что отзыв удален
            assert not db_helper.review_exists_by_ids(
                created_review.movie_id, 
                created_review.user_id
            )
            
            # Создаем новый отзыв для тестирования удаления по ID
            new_review = db_helper.create_test_review(review_data)
            
            # Удаляем отзыв по ID
            db_helper.delete_review_by_ids(
                new_review.movie_id, 
                new_review.user_id
            )
